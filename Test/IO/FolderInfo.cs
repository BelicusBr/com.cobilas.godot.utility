...
public sealed class FolderInfo(string? path) : IFolderInfo {
	private bool disposedValue;
    private string? path = TreatPath(path);
    private KeyValuePair<string, string>[][] datas = GetDatas(path);

    public string FullName => path!;
    public TDSIDataInfo Parent => Folder.GetParent(this);
    ...
    public IArchiveInfo CreateArchive(string? name) {
    ...
    public IFolderInfo CreateFolder(string? name) {
        if (name is null) throw new ArgumentNullException(nameof(name));
        _ReadOnlyException();
        string path = GodotPath.Combine(GodotPath.GlobalizePath(FullName), name);
    ...
    public IArchiveInfo[] GetArchives() {
        ...
        for (long I = 0; I < result.Length; I++)
            result[I] = new ArchiveInfo(_Archives[I].Value);
        return result;
    }
    ...
    public IFolderInfo[] GetFolders() {
        ...
        for (long I = 0; I < result.Length; I++)
            result[I] = new FolderInfo(_Folder[I].Value);
        return result;
    }
	...
	public bool MoveTo(string? name, string? path) {
        ...
        for (long I = 0; I < 2; I++)
		    for (long J = 0; J < ArrayManipulation.ArrayLongLength(datas[I]); J++)
			    if (datas[I][J].Value == name) {
				    ...
				    IOFile.Move(oldPath, newPath);
                    datas[I] = ArrayManipulation.Remove(J, datas[I]);
					return true;
			    }
		return false;
	}
    ...
    public void Refresh() {
        ...
        datas = GetDatas(FullName);
    }

    public IEnumerator<TDSIDataInfo> GetEnumerator() {
        for (byte I = 0; I < 2; I++)
            foreach (KeyValuePair<string, string> item in datas[I]) {
    ...
    private static KeyValuePair<string, string>[][] GetDatas(string? path) {
        if (path is null) throw new ArgumentNullException(nameof(path));
        using GDFolder folder = new();
        Error error;
        switch (error = folder.Open(path)) {
            case Error.Ok:
                folder.ListDirBegin(true, false);
